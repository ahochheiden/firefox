...
# Workaround: SkEdge.cpp and SkAnalyticEdge.cpp both define
# `static inline SkFixed SkFDot6ToFixedDiv2(...)` and
# `static inline SkFDot6 cheap_distance(...)` at file scope. With separate
# TUs `static` gives them internal linkage so each TU has its own copy;
# inside a single unified TU the two definitions collide. Cap unification
# in this directory until the upstream files put those helpers in an
# anonymous namespace, give them per-file unique names, or share a
# common header.
