...
argv layout: ``TOPOBJDIR DEPFILE FILE_GENERATE_ARGS...``
...
def main(argv):
    if len(argv) < 3:
        print(
            "usage: pygen_runner TOPOBJDIR DEPFILE FILE_GENERATE_ARGS...",
            file=sys.stderr,
        )
        return 2
    topobjdir = argv[0]
    depfile = argv[1]
    rc = file_generate.main(argv[2:])
    if rc:
        return rc
    return filter_depfile.main([depfile, topobjdir])
