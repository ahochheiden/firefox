...
class NinjaBackend(CommonBackend):
    ...
    def consume_object(self, obj):
        ...
        if isinstance(obj, UnifiedSources):
            ...
            self._early_compile_dirs.add(relobjdir)
    ...
    def _emit_compile_statements(self, writer):
        ...
        for g in self._generated_files:
            ...
            include_all = g.required_before_export or is_cbindgen or is_define_file
            has_rs = any(
        ...
        dist_include_prefix_early = mozpath.join(self._topobjdir, "dist/include") + "/"
        for o in self._webidl_outputs:
            if o.startswith(dist_include_prefix_early):
        ...
        early_base_cats = ("base-exports", "base-core")

        def _categories_for(reldir):
            cats = list(
                early_base_cats
                if reldir in self._early_compile_dirs
                else full_base_cats
            )
    ...
    def _emit_post_link_stamps(self, writer, out_path, final_target):
        ...
        if substs.get("ENABLE_STRIP"):
            strip_flags = " ".join(
                self._rarg(f) for f in substs.get("STRIP_FLAGS") or []
            )
                ...
                for prefix in symbol_file_prefixes:
                    if f.startswith(prefix):
                        ...
                        if rel and not os.path.isabs(rel):
                            abs_path = mozpath.normpath(
                                mozpath.join(lib.objdir, rel)
                            )
    ...
    def _emit_jar_statements(self, writer):
        ...
        for jar in self._jar_manifests:
            ...
            # pass through as-is.
            defines_parts = [self._rarg(d) for d in per_dir_defines]
            if acdefines:
