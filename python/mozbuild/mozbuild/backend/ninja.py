...
class NinjaBackend(CommonBackend):
    ...
    def _emit_compile_statements(self, writer):
        ...
        for name, buckets in categories.items():
            writer.build(
                f"$topobjdir/.ninja-headers-{name}",
                "phony",
                inputs=[n_path(o) for o in buckets["all"]] if buckets["all"] else None,
            )
            writer.build(
                f"$topobjdir/.ninja-headers-{name}-host",
                "phony",
                inputs=(
                    [n_path(o) for o in buckets["host"]] if buckets["host"] else None
                ),
            )
        ...
        writer.build(
            ".ninja-generated",
            "phony",
            inputs=[f"$topobjdir/.ninja-headers-{name}" for name in categories],
        )
        writer.build(
    ...
    def _emit_generated_file_statements(self, writer):
        ...
        for g in self._generated_files:
            ...
            extra_deps = [
                mozpath.normsep(d.full_path)
                for d in getattr(g, "extra_deps", ()) or ()
            ]
            ...
            implicit.extend(self._n_rel(d) for d in extra_deps)
            writer.build(
