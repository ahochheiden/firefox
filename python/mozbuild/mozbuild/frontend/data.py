...
class EarlyCompile(ContextDerived):
    """Marks a directory as opt-in for early compile scheduling.

    Backends may relax the prereq fence for compile edges in this
    directory: source-tree EXPORTS and core preprocessed config files
    must still be installed first, but XPIDL and non-core generated
    dist/include headers are not waited on.
    """
