...
# so consumers resolve the binary at `$MOZ_FETCHES_DIR/ninja/bin/ninja[.exe]`.
...
case "$1" in
    ...
    --win64-aarch64)
        ...
        # `win64-ninja-fork` artifact instead.
    ...
    --linux-aarch64)
        ...
        # build with the host `linux64-ninja-fork` artifact.
    ...
    *)
        ...
        # docker image (matches the linux64-gn pattern). No clang
        # toolchain is fetched in this task.
        export CC=gcc
        export CXX=g++
        cd $SRC_DIR
        python3 configure.py --bootstrap
...
# Stage the binary at `ninja/bin/ninja[.exe]` so consumers resolve it
# at `$MOZ_FETCHES_DIR/ninja/bin/ninja[.exe]`.
mkdir -p $STAGE/bin
