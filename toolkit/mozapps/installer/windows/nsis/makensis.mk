...
USE_UPX := $(if $(NSIS_USE_UPX),--use-upx)
...
NSIS_STAGE_ARGS = --config-dir=$(CONFIG_DIR) \
  ...
  --topsrcdir=$(topsrcdir) $(addprefix --locale-arg=,$(PPL_LOCALE_ARGS)) --ab-cd=$(AB_CD) --preprocess-locale \
  $(NSIS_INSTALLER_DEFINES) $(ACDEFINES) -DTOPOBJDIR=$(topobjdir)

$(CONFIG_DIR)/setup.exe::
ifdef MOZ_USE_LEGACY_INSTALLER
...
ifdef MOZ_USE_LEGACY_INSTALLER
	...
	$(call py_action,nsis_stage maintenanceservice,$(NSIS_STAGE_ARGS))
	$(call py_action,nsis_build maintenanceservice_installer,--config-dir=$(CONFIG_DIR) \
	--nsi=maintenanceservice_installer.nsi --makensis=$(MAKENSISU) $(addprefix --makensis-flag=,$(MAKENSISU_FLAGS)) \
	--produced=maintenanceservice_installer.exe --output=$(DIST)/bin/maintenanceservice_installer.exe)
endif
endif
